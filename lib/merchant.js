...
class Merchant {
    constructor(config, options) {
        ...
        this.Paynl = require('./core');
        this.paynl = this.Paynl.init(this.username, this.password, this.signature, this.callback.success, this.callback.error, this.debug);
    }
    ...
    async createPaymet(options) {
        ...
        return await new Promise((resolve, reject) => {
            this.paynl.pay(options.order_id, form.amount, 'TopUp', form.currency, (err, url) => {
                if (err)
                    return reject(err);
                return resolve({url})
            });
    ...
    checkPayment(options) {
        return new Promise((resolve, reject) => {
            this.paynl.detail(options.token, options.payer, function (err, data, invoiceNumber, price, custom_data_array) {
                if (err) {
                    console.log(err);
                    return;
                }
